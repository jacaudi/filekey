...
function displayPublicKey(main_inner) {
    ...
    getDetEcdhPublicKey(function(ret) {
        ...
        if (ret == null)
            ...
            var copy_icon = hb.getSvg("copy_icon", {
                class_string: "copy_icon"
            });
            html_string = ` <span>Your share key is a public key that allows others to encrypt data that only you can decrypt:</span><p class=word_broken>${temp_pub}</p><div class=copy_button id=${rand_copy_id} class=no_select><span id=${rand_svg_id}></span><span id=${rand_pub_id}>Copy</span></div>`;
        }
        htmlWriter({
            char_speed: 8
        }, html_string, main_inner, events_obj, function(ret) {
